...
UnifiedRecord TimeBasedCsvWriter::parseRecord(const std::string& protocol, const std::string& csv_line) {
    ...
    if (protocol == "arp") {
        ...
        // @timestamp,smac,dmac,sip,sp,dip,dp,prid,dir,hdr.companyId,hdr.plcinfo,hdr.cpuinfo,hdr.source,hdr.len,hdr.fenetpos,inst.cmd,inst.dtype,inst.blkcnt,inst.errstat,inst.errinfo,inst.vars,inst.datasize,inst.data,translated_addr,description
        if (fields.size() > 1) record.smac = fields[1];
        if (fields.size() > 2) record.dmac = fields[2];
        if (fields.size() > 3) record.sip = fields[3];
        if (fields.size() > 4) record.sp = fields[4];
