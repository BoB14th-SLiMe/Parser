...
TimeBasedCsvWriter::TimeBasedCsvWriter(const std::string& output_dir, int interval_minutes, AssetManager* assetManager)
    : m_output_dir(output_dir), m_interval_minutes(interval_minutes), m_assetManager(assetManager) {
    #ifdef _WIN32
...
UnifiedRecord TimeBasedCsvWriter::parseRecord(const std::string& protocol, const std::string& csv_line) {
    ...
    // 자산 정보 매핑 (sip, dip가 있는 프로토콜만)
    if (m_assetManager) {
        if (protocol == "arp") {
            if (fields.size() > 4) {
                record.src_device = m_assetManager->getDeviceName(fields[4]); // sip
            }
            if (fields.size() > 6) {
                record.dst_device = m_assetManager->getDeviceName(fields[6]); // tip
            }
        } else {
            // 대부분의 프로토콜: sip는 3번, dip는 5번 인덱스
            if (fields.size() > 3) {
                record.src_device = m_assetManager->getDeviceName(fields[3]);
            }
            if (fields.size() > 5) {
                record.dst_device = m_assetManager->getDeviceName(fields[5]);
            }
        }
    }
    ...
    if (protocol == "arp") {
        ...
        if (fields.size() > 7) record.sq = fields[7];
        if (fields.size() > 8) record.ak = fields[8];
        if (fields.size() > 9) record.fl = fields[9];
        ...
        if (fields.size() > 5) record.dip = fields[5];
        if (fields.size() > 6) record.dp = fields[6];
        if (fields.size() > 7) record.xgt_prid = fields[7];
        if (fields.size() > 8) record.dir = fields[8];
        if (fields.size() > 9) record.xgt_companyId = fields[9];
        if (fields.size() > 10) record.xgt_plcinfo = fields[10];
        if (fields.size() > 11) record.xgt_cpuinfo = fields[11];
        if (fields.size() > 12) record.xgt_source = fields[12];
        if (fields.size() > 13) record.xgt_len = fields[13];
        if (fields.size() > 14) record.xgt_fenetpos = fields[14];
        if (fields.size() > 15) record.xgt_cmd = fields[15];
        if (fields.size() > 16) record.xgt_dtype = fields[16];
        if (fields.size() > 17) record.xgt_blkcnt = fields[17];
        if (fields.size() > 18) record.xgt_errstat = fields[18];
        if (fields.size() > 19) record.xgt_errinfo = fields[19];
        if (fields.size() > 20) record.xgt_vars = fields[20];
        if (fields.size() > 21) record.xgt_datasize = fields[21];
        if (fields.size() > 22) record.xgt_data = fields[22];
        if (fields.size() > 23) record.xgt_translated_addr = fields[23];
        if (fields.size() > 24) record.xgt_description = fields[24];
    }
...
void TimeBasedCsvWriter::writeUnifiedHeader(std::ofstream& out) {
    out << "@timestamp,protocol,smac,dmac,sip,sp,dip,dp,sq,ak,fl,dir,"
        << "src_device,dst_device,"  // 자산 정보 추가
...
void TimeBasedCsvWriter::writeTimeSlot(const std::string& time_slot) {
    ...
    for (const auto& record : records) {
        out << escapeCSV(record.timestamp) << ","
            ...
            << escapeCSV(record.src_device) << ","  // 자산 정보
            << escapeCSV(record.dst_device) << ","  // 자산 정보
